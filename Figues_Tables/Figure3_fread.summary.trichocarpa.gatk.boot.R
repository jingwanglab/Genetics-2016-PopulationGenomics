...
f=function(count,number){  ###count is the table and n is the number of sites for either 4-fold or 0-fold sites
        ...
        maf=as.integer(count_sample$V6*48)
	sfs_table=as.numeric(table(maf))
        sfs=rep(0,49)
        for (i in 1:25){sfs[i]=sfs_table[i]+sfs_table[50-i]}
	sfs[25]=sfs[25]/2
	sum=sum(sfs)
	sfs[1]=number-sum+sfs[1]
        fixed_table=length(which(count_sample$V7=="fixed"))
...
###1.0-fold
Outfile=paste("/proj/b2010014/GenomePaper/population_genetics/pan_genome/shared/three_species/zero_fold/dfe/trichocarpa/bootstrap/bootstrap",bootstrap_n,"/trichocarpa.zero_fold.bootstrap",bootstrap_n,".txt",sep="")
cat("1",file=Outfile)
cat("\n",file=Outfile,append=T)
cat("48",file=Outfile,append=T)
cat("\n",file=Outfile,append=T)
...
Outfile_omega=paste("/proj/b2010014/GenomePaper/population_genetics/pan_genome/shared/three_species/zero_fold/dfe/trichocarpa/bootstrap/bootstrap",bootstrap_n,"/trichocarpa.zero_fold.bootstrap",bootstrap_n,".omega.txt",sep="")
